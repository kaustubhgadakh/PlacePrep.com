...
window.onload = () => {
    emailjs.init("hcdI0IYnnpZH1LOX2");
}
forgotPasswordBtn.addEventListener('click', async () => {
    ...
    console.log(user);
    if (user) {
        emailjs.send("service_t7pt8z5", "template_vg8rq5r", { from_name: 'PlacePrep', to_name: user.name, to_email: email, the_pass: user.password, })
...
signupSubmitBtn.addEventListener('click', (e) => {
    ...
    emailjs.send("service_t7pt8z5", "template_5e59gpr", { from_name: 'PlacePrep', to_name: name, to_email: email, the_otp: otpToSend, reply_to: email })
        .then(function (response) {
            console.log('SUCCESS!', response.status, response.text);
        }, function (error) {
            console.log('FAILED...', error);
